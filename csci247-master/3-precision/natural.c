...
natural_t add_naturals(natural_t left, natural_t right)
{
   ...
   /* 3 */
   //bug was ++i was out of position and incorrect it should be i++
...
natural_t read_natural(FILE* source)
{
   ...
   if (nat.byte_count > 0) {
      ...
      //length needed to change instead of being 1 the entire time so we peek the stack
      //till there is no more length to do so
      while (length > 0) {
...
{
   //bug here was this was unsigned but should be an int
   int i = natural.byte_count - 1;
   if (natural.byte_count > 0)
      printf("%x", natural.bytes[i]);
   else
      printf("0");
   //issue here was the most significant byte was being printed twice which is solved
   //by educing i by 1 which makes it iterate one less time cutting out the most significant
   //byte
   i--;
   for (; i >= 0; i--){
