...
// I use recursion to build a string to compare to the secret
char* builder (char*, char*);
bool matches(char* line) {
  char* secret = "1 1 5";
  char* lineBuilder = strtok(line, " ");
  builder(line, lineBuilder);
  return ~strcmp(secret, lineBuilder);
}
char* builder(char* line, char* lineBuilder) {
  if (strtok(NULL, " ") == NULL) {
    return lineBuilder;
  }
  else {
    builder(line, strcat(lineBuilder, strtok(NULL, " ")));
  }
}
